        ...
        <div className="dish_box shadow-sm" >
            <div className="card border-0 rounded">
                ...
                <div className="card-body pb-0">
                    <h6 className="mb-1" data-bs-toggle="modal" data-bs-target="#dishModal" onClick={() => selectDish(dish)}>{dish.name}</h6>
                    <div className="col"><p className="text-secondary price mb-0"><i className="fas fa-rupee-sign"></i> {dish.price}</p></div>

                </div>
                <button onClick={() => addToTable(dish)} className="btn btn-link text-danger text-decoration-none">Add to Table</button>
            </div>
        </div>
    )
}
